...
<p align="center">
  🤝 对抗式谈判模拟、态势分析、复盘总结与博弈分叉树推演引擎，支持本地自定义场景与 API 配置
</p>
...
除了直接运行内置场景之外，项目还提供前端内置的 `Local Setup` 面板，允许你在本地直接编辑场景 JSON、保存自定义谈判场景，并修改 API Provider、Base URL、Model、Key 等配置。也就是说，你不需要先改源码再重启前端，很多本地实验都可以在界面里直接完成。
...
- 前端内置 `Local Setup` 面板，可直接维护本地 API 配置
- 可以切换或填写 DeepSeek、OpenAI Compatible、Gemini 的 Key、Base URL、Model
- 可以直接编辑并保存本地场景 JSON，写入 `backend/scenarios/*.json`
- API 配置保存后重启后端即可生效；场景文件保存后可立即在首页场景面板中刷新使用
...
如果你想把它当作一个可本地调参、可快速改场景的实验台来用，可以直接在前端右上角打开 `本地设置 / Local Setup`：

1. 在 `API 设置` 页签里填写或切换 provider、API Key、Base URL、Model
2. 保存后重启后端，让新的模型配置从 `backend/.env` 重新加载
3. 在 `场景文件` 页签里编辑 JSON 模板，保存到 `backend/scenarios/*.json`
4. 刷新首页后，即可在场景列表里看到你自定义的本地场景
